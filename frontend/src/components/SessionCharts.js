...
//   // -----------------------------
//   // If the backend is returning the real name in "playerName", we use that.
//   // If your real name is in "p.name" or "p.displayName", swap accordingly.
//   const allPlayerNames = useMemo(() => {
//     return Array.from(
//       new Set(playerDataArray.map((p) => p.playerName)) // <--- "playerName" should be the real name now
//     );
//   }, [playerDataArray]);
...
//           pdfHeight - headerHeight - margin - (numCharts - 1) * gap;
...
//               />
//               {/* Show the player's name instead of ID */}
...
//                 } else {
//                   const foundSplit = p.splitPlayerMetrics?.find(
//                     (sp) => sp.SplitNumber === splitVal
//                   );
//                   if (!foundSplit) {
//                     val = NaN;
//                   } else {
...
//                   }
//                 }
//                 rows.push([p.playerName, val]); // <-- Using p.playerName for display
//               });
//               const filtered = [
//                 rows[0],
//                 ...rows.slice(1).filter(
...
function SessionCharts({
  ...
  // 4) Player Names & Visibility
  // -----------------------------
  // We assume your backend sets p.playerName = "D.O'Hagan" (the real name).
  const allPlayerNames = useMemo(() => {
    return Array.from(
      new Set(playerDataArray.map((p) => p.playerName))
    );
  }, [playerDataArray]);
          ...
          return (
            <div key={name}>
              <input
                ...
                onChange={() => toggleVisibility(name)}
              />
              {/* This label shows the player's name next to the checkbox */}
              <label htmlFor={inputId}>{name}</label>
            </div>
          );
        })}
