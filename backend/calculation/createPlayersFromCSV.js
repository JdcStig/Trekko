...
const createPlayersFromCSV = async (sessionId, userId) => {
  ...
  try {
    ...
    // Process each SessionPlayerData entry to create players if missing.
    for (const data of sessionPlayerDataList) {
      const playerIdentifier = data.playerId;
      const randomPosition = positions[Math.floor(Math.random() * positions.length)];
      ...
      if (!existingPlayer) {
        ...
        // Emit a socket event so the frontend can display a toast.
        getIO().emit('playerCreated', { playerName: newPlayer.name });
      }
    }

    console.log(`✅ createPlayersFromCSV done. Created: ${createdPlayersSet.size} new players.`);
  } catch (error) {
