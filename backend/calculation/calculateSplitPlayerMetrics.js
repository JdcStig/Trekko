...
const calculateSplitPlayerMetrics = (speeds = [], splits = []) => {
  ...
  return splits.map((split, index) => {
    // Convert milliseconds to seconds for proper duration calculation
    const splitLengthSeconds = (split.end - split.start) / 1000;
    // 10 readings per second => expected number of readings for this split
    const splitSize = Math.round(splitLengthSeconds * 10);

    const startIndex = currentIndex;
    const endIndex = currentIndex + splitSize;

    // Extract speeds for this split
    const splitSpeeds = speeds.slice(startIndex, endIndex);
