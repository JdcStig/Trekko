...
const deleteTeam = asyncHandler(async (req, res) => {
  ...
  if (team) {
    // Check if there are any players attached to this team.
    // Adjust the query field(s) as needed to match your player model.
    const attachedPlayer = await Player.findOne({ teamName: team.name, userId: team.userId });
    if (attachedPlayer) {
      res.status(400);
      throw new Error('Cannot delete team as it has players attached.');
    }
